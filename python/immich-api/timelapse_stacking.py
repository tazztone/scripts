...
LOG_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "timelapse_stacking_last_run.json"
)
...
def get_candidates(options: ProcessOptions):
    ...
    for seq in sequences:
        ...
        if ok_span and ok_regular and ok_size and ok_loc:
            candidates.append(
                {"assets": seq, "span": span, "cv_gap": cv_gap, "cv_size": cv_size}
            )
    ...
    for c in candidates:
        ...
        for kept in merged:
            ...
            overlap = (
                intersection / min(len(c_ids), len(kept_ids)) if intersection else 0
            )
            if overlap >= 0.80:
                merged_into = kept
                break
        ...
        if merged_into is not None:
            ...
            if new_frames:
                ...
                times = [
                    datetime.fromisoformat(a["localDateTime"])
                    for a in merged_into["assets"]
                ]
                gaps = [
                ...
                merged_into["cv_gap"] = (
                    statistics.stdev(gaps) / statistics.mean(gaps)
                    if len(gaps) > 1
                    else 0
                )
                sizes = [
                    a.get("exifInfo", {}).get("fileSizeInByte", 0)
                    for a in merged_into["assets"]
                ]
...
def create_verify_albums(candidates, count):
    ...
    for i, c in enumerate(to_verify):
        ...
        album = requests.post(
            f"{BASE_URL}/api/albums", headers=headers, json={"albumName": name}
        ).json()
        if "id" not in album:
            print(f"    [!] Failed: {name}")
            continue
...
def apply_stacks(candidates):
    ...
    def process_candidate(i, c):
        ids = [a["id"] for a in c["assets"]]
        resp = session.post(f"{BASE_URL}/api/stacks", json={"assetIds": ids})
        return i, c, ids, resp

    with ThreadPoolExecutor(max_workers=min(10, len(candidates) or 1)) as executor:
        futures = {
            executor.submit(process_candidate, i, c): (i, c)
            for i, c in enumerate(candidates)
        }

        results = [None] * len(candidates)
        for future in as_completed(futures):
            i, c, ids, resp = future.result()
            results[i] = (c, ids, resp)

    for i, (c, ids, resp) in enumerate(results):
...
def create_index_album(candidates, mode="index"):
    ...
    if existing:
        ...
        album = requests.post(
            f"{BASE_URL}/api/albums", headers=headers, json={"albumName": album_name}
        ).json()
        if "id" not in album:
        ...
        if targets:
            ...
            if ask("Delete this album as well?", {"Y": "Yes", "N": "No"}) == "Y":
                resp = requests.delete(
                    f"{BASE_URL}/api/albums/{targets[0]['id']}", headers=headers
                )
                if resp.status_code == 204:
                    print("    ✓ Album deleted.")
                else:

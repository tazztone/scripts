...
def same_location(assets, max_dist_deg=0.0005):
    coords = [(a.get("exifInfo", {}).get("latitude"), a.get("exifInfo", {}).get("longitude"))
              for a in assets if a.get("exifInfo", {}).get("latitude")]
    if len(coords) < 2:
...
if DETECTION_SOURCE == "duplicates":
    ...
    if unstacked:
        ...
        for prev, curr in zip(unstacked, unstacked[1:]):
            gap = (datetime.fromisoformat(curr["localDateTime"]) -
                   datetime.fromisoformat(prev["localDateTime"])).total_seconds()
...
for i, (seq, stats) in enumerate(timelapses):
    ...
    print(f"  [{i+1}] {len(seq):4d} frames | {start[:19]} | span {stats['span']:.0f}s | "
          f"cv_gap={stats['cv_gap']:.3f} | cv_size={stats['cv_size']:.3f}")

# Phase 3: Stacking
if DRY_RUN:
    print(f"\nDRY RUN — {len(timelapses)} stacks would be created. Set DRY_RUN = False in the script to apply.")
else:
    print(f"\nStacking {len(timelapses)} sequences...")
    for i, (seq, stats) in enumerate(timelapses):
        ids = [a["id"] for a in seq]
        resp = requests.post(f"{BASE_URL}/api/stacks", headers=headers, json={"assetIds": ids})
        status = "✓" if resp.status_code == 201 else f"✗ ({resp.status_code})"
        print(f"  [{i+1}] {status} {len(ids)} frames @ {seq[0]['localDateTime'][:19]}")
    print("Done!")

...
get_video_codec_info() {
    ...
    if probe_media "$file"; then
        echo "VIDEO:${PROBED_INFO[video_codec]:-}:${PROBED_INFO[video_profile]:-}:${PROBED_INFO[video_level]:-}:${PROBED_INFO[video_pix_fmt]:-}:${PROBED_INFO[width]:-}:${PROBED_INFO[height]:-}:${PROBED_INFO[video_fps]:-}"
    else
        echo "VIDEO:::::::"
    fi
}
...
get_audio_codec_info() {
    local file="$1"
    if probe_media "$file"; then
        echo "AUDIO:${PROBED_INFO[audio_codec]:-}:${PROBED_INFO[audio_sample_rate]:-}:${PROBED_INFO[audio_channels]:-}:${PROBED_INFO[audio_channel_layout]:-}"
    else
        echo "AUDIO:::"
    fi
}

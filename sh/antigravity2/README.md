...
* **Desktop App (Profile 2):** Launched via `bwrap` with `--no-sandbox` and `~/.antigravity-cli-account2/.gemini` mounted over `~/.gemini`.
  ...
  * `oauth_creds.json` (seeded with Google `id_token`, `access_token`, and `refresh_token`) and a relative symlink (`antigravity/antigravity-oauth-token` → `../antigravity-cli/antigravity-oauth-token`) allow the backend daemon to start directly authenticated as Account 2 without prompting or browser roundtrips.

...
# Test 9b: Extreme Speed (4x) -> Triggers atempo chaining
...
Scale / Resize|Output Format
 (Inactive)||720p|| (Inactive)|| (Inactive)|||||||Medium (CRF 23)|||H.265|Use NVENC (Nvidia)||
EOF
run_test "ffmpeg/🧰 Universal-Toolbox.sh" "vcodec=hevc,tags=_nvenc" "$TEST_DATA/input.mp4" || FAILED=$((FAILED+1))
...
cat <<EOF > /tmp/zenity_responses
Scale / Resize
 (Inactive)||720p|| (Inactive)|| (Inactive)|||||||Medium (CRF 23)|||Auto/MP4|None (CPU Only)||
 (Inactive)||720p|| (Inactive)|| (Inactive)|||||||Medium (CRF 23)|||Auto/MP4|None (CPU Only)||
EOF

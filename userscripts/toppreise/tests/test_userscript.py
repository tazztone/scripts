...
def test_competing_reference_price_resolves_to_green_low(page: Page):
    ...
    page.evaluate(
        "() => { window.ToppreiseSuite.CONFIG.REAL_DEAL_FILTER_ACTIVE = true; }"
    )
    ...
    def handle_pricechart(route):
        ...
        if (
            "1003795" in (route.request.post_data or "")
            or "p_pc_pid=1003795" in route.request.url
        ):
            route.fulfill(
                status=200,
...
def test_exact_cent_boundary_badge_states(page: Page):
    ...
    page.evaluate(
        "() => { window.ToppreiseSuite.CONFIG.REAL_DEAL_FILTER_ACTIVE = false; }"
    )
    ...
    cases = [
        ...
        (
            37.94,
            "Neuer Allzeit-Tiefstpreis (CHF 37.94)",
            "tp-deal-alltime-low",
            "tp-deal-not-low",
        ),  # new low
        (
            37.95,
            "Allzeit-Tiefstpreis (CHF 37.95)",
            "tp-deal-alltime-low",
            "tp-deal-not-low",
        ),  # at low
        (
            37.96,
            "Historischer Tiefstpreis lag bei CHF 37.95",
            "tp-deal-not-low",
            "tp-deal-alltime-low",
        ),  # above low
        (
    ...
    for curr_price, title_match, expected_class, unexpected_class in cases:
        ...
        window.ToppreiseSuite?.memoryCache?.set('1003795', JSON.parse(JSON.stringify({{
                tiefstpreis: 37.95,
                hoechstpreis: 55.00,
                medianPrice: 45.00,
                previousLow: 47.82,
        ...
        assert expected_class in badge_class, (
            f"Expected {expected_class} but got {badge_class} for price {curr_price}"
        )
        assert unexpected_class not in badge_class, (
            f"Did not expect {unexpected_class} but got it for price {curr_price}"
        )

        # Verify title string logic
        title = badge.get_attribute("title") or ""
        assert title_match in title, (
            f"Expected {title_match} in {title} for price {curr_price}"
        )
...
def test_best_price_highlighting_and_dimming(page: Page):
    ...
    assert "tp-is-cheapest" in (
        page.locator("#card-cheapest").get_attribute("class") or ""
    )
    assert page.locator("#card-cheapest .tp-best-price-badge").is_visible()

    # Card 2 is more expensive -> not cheapest
    assert "tp-not-cheapest" in (
        page.locator("#card-expensive").get_attribute("class") or ""
    )
...
def test_negative_keywords_filtering(page: Page):
    ...
    assert "tp-negative-filtered" in (
        page.locator("#card-negative").get_attribute("class") or ""
    )
    assert "tp-negative-filtered" not in (
        page.locator("#card-cheapest").get_attribute("class") or ""
    )
...
def test_min_offers_filter(page: Page):
    ...
    assert "tp-min-offers-filtered" in (
        page.locator("#card-low-offers").get_attribute("class") or ""
    )
    assert "tp-min-offers-filtered" not in (
        page.locator("#card-cheapest").get_attribute("class") or ""
    )
...
def test_price_alarm_automation(page: Page):
    ...
    assert (
        page.locator("#mock-alarm-dialog").get_attribute("data-dialog-closed") == "true"
    )
...
def test_card_quick_block_button_and_toast_undo(page: Page):
    ...
    assert "tp-category-filtered" in (
        page.locator("#card-cheapest").get_attribute("class") or ""
    )
    ...
    assert "tp-category-filtered" not in (
        page.locator("#card-cheapest").get_attribute("class") or ""
    )
...
def test_modal_mode_and_settings_in_shadow_dom(page: Page):
    ...
    page.wait_for_selector("#tp-root >> #tp-settings-dialog", state="hidden")
    has_mode_hide = page.evaluate(
        "() => document.body.classList.contains('tp-mode-hide')"
    )
...
def test_master_filter_toggle_and_category_preservation(page: Page):
    ...
    page.wait_for_selector(
        "#card-low-offers:not(.tp-min-offers-filtered)", state="visible"
    )
    assert "tp-negative-filtered" not in (
        page.locator("#card-negative").get_attribute("class") or ""
    )
...
def test_discount_heatmap_toolbar_toggle(page: Page):
    ...
    assert "tp-heatmap-active" not in (
        page.locator("#card-cheapest").get_attribute("class") or ""
    )
...
def test_darkreader_dynamic_mode_compatibility(page: Page):
    ...
    dr_bgimage = card.evaluate(
        "el => el.style.getPropertyValue('--darkreader-inline-bgimage')"
    )
    dr_bgcolor = card.evaluate(
        "el => el.style.getPropertyValue('--darkreader-inline-bgcolor')"
    )
    ...
    child_bg = product_name.evaluate(
...
def test_real_deal_filter_non_bestpreis_toggle(page: Page):
    ...
    def handle_pricechart(route):
        ...
        if "p_pc_pid=797571" in url:
            ...
            route.fulfill(
                ...
                headers={"access-control-allow-origin": "*"},
                content_type="text/html",
                body='<div class="PriceChartLegend"><div class="title">Tiefstpreis</div><div class="Plugin_Price">10.00</div></div>',
            )
        else:
            route.fulfill(
                status=200,
                headers={"access-control-allow-origin": "*"},
                content_type="text/html",
                body='<div class="PriceChartLegend"><div class="title">Tiefstpreis</div><div class="Plugin_Price">999.00</div></div>',
            )
    ...
    assert "tp-non-bestpreis-filtered" in (
        page.locator("#card-negative").get_attribute("class") or ""
    )
...
def test_real_deal_dom_memoization_and_cache_pruning(page: Page):
    ...
    page.evaluate("""() => {
        ...
        window.ToppreiseSuite?.memoryCache?.set('stale999', JSON.parse(JSON.stringify({ tiefstpreis: 50, hoechstpreis: 100, time: staleTime })));
        localStorage.setItem('tp_hist_v1_fresh999', JSON.stringify({ tiefstpreis: 80, hoechstpreis: 120, time: freshTime }));
        window.ToppreiseSuite?.memoryCache?.set('fresh999', JSON.parse(JSON.stringify({ tiefstpreis: 80, hoechstpreis: 120, time: freshTime })));
    }""")

    # Trigger setCachedPriceStats by mocking a route and clicking check badge
    page.route(
        "**/plugins/product/pricechart*",
    ...
    stale_exists = page.evaluate(
        "() => localStorage.getItem('tp_hist_v1_stale999') !== null"
    )
    fresh_exists = page.evaluate(
        "() => localStorage.getItem('tp_hist_v1_fresh999') !== null"
    )
...
def test_real_deal_batch_check_button_counter_and_run(page: Page):
    ...
    page.wait_for_function(
        "() => !document.querySelector('#tp-bar-batch-check-btn').classList.contains('tp-batch-active')"
    )
...
def test_empty_state_notice_and_actions(page: Page):
    ...
    assert page.locator("#card-cheapest").is_visible()
    assert (
        page.evaluate("() => window.ToppreiseSuite.CONFIG.FILTER_NEG_ENABLED") is False
    )
...
def test_product_detail_page_deal_badge(page: Page):
    ...
    page.route(
        "**/plugins/product/pricechart*840582*",
        lambda route: route.fulfill(
            status=200,
            headers={"access-control-allow-origin": "*"},
            content_type="text/html",
...
def test_real_world_toppreise_pricechart_html_parsing(page: Page):
    ...
    page.route(
        "**/plugins/product/pricechart*456*",
        lambda route: route.fulfill(
            status=200,
            headers={"access-control-allow-origin": "*"},
            content_type="text/html",
    ...
    page.click("#card-negative .badge-dif")
    page.wait_for_selector(
        "#card-negative .badge-dif.tp-deal-alltime-low", timeout=3000
    )
...
def test_slash_key_focuses_negative_filter(page: Page):
    ...
    is_focused = page.evaluate(
        "() => document.activeElement?.id === 'tp-inline-negative-input'"
    )
    assert is_focused
...
def test_inline_negative_input_clear_button(page: Page):
    ...
    assert (
        page.evaluate("() => window.ToppreiseSuite?.CONFIG?.NEGATIVE_TERMS")
        == "QuickClearTest"
    )
...
def test_negative_caching_and_manual_click_override(page: Page):
    ...
    page.evaluate("""() => {
        ...
        window.ToppreiseSuite?.memoryCache?.set('797571', JSON.parse(JSON.stringify({ unavailable: true, time: Date.now() })));
    }""")

    # Batch check ignores negatively cached card
    cached = page.evaluate(
        "() => window.ToppreiseSuite?.CONFIG ? localStorage.getItem('tp_hist_v1_797571') : null"
    )
    assert "unavailable" in (cached or "")

    # Manual click bypasses negative cache and fetches fresh stats
    page.route(
        "**/plugins/product/pricechart*797571*",
        lambda route: route.fulfill(
            status=200,
            headers={"access-control-allow-origin": "*"},
            content_type="text/html",
...
def test_sparklines_beta_settings_toggle(page: Page):
    ...
    assert (
        page.evaluate("() => window.ToppreiseSuite?.CONFIG?.ENABLE_SPARKLINES") is True
    )
...
def test_real_deal_record_low_with_previous_low_subline(page: Page):
    ...
    def handle_pricechart_post(route):
        if route.request.method == "POST":
            ...
            series_data = [
                [
                    [1672531199000, 2500.0],
                    [1675209599000, 2200.0],
                    [1677628799000, 1800.0],
                ],
                [
                    [1672531199000, 2500.0],
                    [1675209599000, 2200.0],
...
def test_bestpreise_filter_bar_toggle_and_state(page: Page):
    ...
    assert "tp-bestpreise-bar" in (
        page.locator("#tp-suite-filter-bar").get_attribute("class") or ""
    )
    assert "tp-bestpreise-active" in (btn.get_attribute("class") or "")
    assert (
        page.evaluate("() => window.ToppreiseSuite.CONFIG.BESTPREISE_MODE_ACTIVE")
        is True
    )
    ...
    btn.click()
    assert "tp-bestpreise-bar" not in (
        page.locator("#tp-suite-filter-bar").get_attribute("class") or ""
    )
...
def test_bestpreise_card_heatmap_and_badge(page: Page):
    ...
    page.evaluate("""() => {
        ...
        window.ToppreiseSuite?.memoryCache?.set('797571', JSON.parse(JSON.stringify({
            tiefstpreis: 1800,
            hoechstpreis: 2600,
            medianPrice: 2400,
            previousLow: 2200,
        ...
        window.ToppreiseSuite?.memoryCache?.set('797572', JSON.parse(JSON.stringify({
            tiefstpreis: 1100,
            hoechstpreis: 1800,
            medianPrice: 1500,
            isNewAllTimeLow: false,
        ...
        window.ToppreiseSuite?.memoryCache?.set('797573', JSON.parse(JSON.stringify({
            tiefstpreis: 10,
            hoechstpreis: 25,
            isNewAllTimeLow: false,
            dataPointCount: 10,
    ...
    card1_subline = page.locator(
        "#card-cheapest .tp-card-historical-price.tp-is-record-low"
    )
    ...
    assert "tp-deal-new-record" not in (
        page.locator("#card-cheapest .badge-dif").get_attribute("class") or ""
    )
...
def test_bestpreise_settings_weight_slider(page: Page):
    ...
    assert (
        page.evaluate("() => window.ToppreiseSuite.CONFIG.BESTPREISE_WEIGHT_RECORD")
        == 0.70
    )
...
def test_parse_price_normalization(page: Page):
    ...
    for input_str, expected in test_cases:
        ...
        assert result == expected, (
            f"Expected parsePrice('{input_str}') to be {expected}, but got {result}"
        )
...
def test_bestpreise_settings_horizon_selection_persistence(page: Page):
    ...
    assert (
        page.evaluate(
            "() => window.ToppreiseSuite.CONFIG.BESTPREISE_MEDIAN_HORIZON_DAYS"
        )
        == 180
    )
...
def test_cache_settings_and_clear_button(page: Page):
    ...
    page.evaluate("""() => {
        ...
        window.ToppreiseSuite?.memoryCache?.set('item1', JSON.parse(JSON.stringify({ tiefstpreis: 100, time: Date.now() })));
        localStorage.setItem('tp_hist_v1_item2', JSON.stringify({ tiefstpreis: 200, time: Date.now() }));
        window.ToppreiseSuite?.memoryCache?.set('item2', JSON.parse(JSON.stringify({ tiefstpreis: 200, time: Date.now() })));
    }""")
    ...
    assert (
        page.evaluate("() => window.ToppreiseSuite.CONFIG.REAL_DEAL_CACHE_HOURS") == 72
    )
...
def test_realistic_page_layout_sidebar_and_tabs_preserved_in_bestpreise_mode(
    page: Page,
):
...
def test_bestpreise_cross_row_sorting_and_natural_order_restoration(page: Page):
    ...
    page.evaluate("""() => {
        ...
        localStorage.setItem('tp_hist_v1_101', JSON.stringify({ tiefstpreis: 100, hoechstpreis: 150, medianPrice: 130, isNewAllTimeLow: false, dataPointCount: 10, time: Date.now() }));
        window.ToppreiseSuite?.memoryCache?.set('101', JSON.parse(JSON.stringify({ tiefstpreis: 100, hoechstpreis: 150, medianPrice: 130, isNewAllTimeLow: false, dataPointCount: 10, time: Date.now() }))); // ~12%
        localStorage.setItem('tp_hist_v1_102', JSON.stringify({ tiefstpreis: 200, hoechstpreis: 250, medianPrice: 235, isNewAllTimeLow: false, dataPointCount: 10, time: Date.now() }));
        window.ToppreiseSuite?.memoryCache?.set('102', JSON.parse(JSON.stringify({ tiefstpreis: 200, hoechstpreis: 250, medianPrice: 235, isNewAllTimeLow: false, dataPointCount: 10, time: Date.now() }))); // ~8%
        localStorage.setItem('tp_hist_v1_103', JSON.stringify({ tiefstpreis: 300, hoechstpreis: 600, medianPrice: 550, previousLow: 480, isNewAllTimeLow: true, realDiscountVsPrevLow: 37, dataPointCount: 10, time: Date.now() }));
        window.ToppreiseSuite?.memoryCache?.set('103', JSON.parse(JSON.stringify({ tiefstpreis: 300, hoechstpreis: 600, medianPrice: 550, previousLow: 480, isNewAllTimeLow: true, realDiscountVsPrevLow: 37, dataPointCount: 10, time: Date.now() }))); // ~37%
        localStorage.setItem('tp_hist_v1_104', JSON.stringify({ tiefstpreis: 400, hoechstpreis: 600, medianPrice: 550, previousLow: 530, isNewAllTimeLow: true, realDiscountVsPrevLow: 25, dataPointCount: 10, time: Date.now() }));
        window.ToppreiseSuite?.memoryCache?.set('104', JSON.parse(JSON.stringify({ tiefstpreis: 400, hoechstpreis: 600, medianPrice: 550, previousLow: 530, isNewAllTimeLow: true, realDiscountVsPrevLow: 25, dataPointCount: 10, time: Date.now() }))); // ~25%
        localStorage.setItem('tp_hist_v1_105', JSON.stringify({ tiefstpreis: 500, hoechstpreis: 800, medianPrice: 800, isNewAllTimeLow: false, dataPointCount: 10, time: Date.now() }));
        window.ToppreiseSuite?.memoryCache?.set('105', JSON.parse(JSON.stringify({ tiefstpreis: 500, hoechstpreis: 800, medianPrice: 800, isNewAllTimeLow: false, dataPointCount: 10, time: Date.now() }))); // ~19%
        localStorage.setItem('tp_hist_v1_106', JSON.stringify({ tiefstpreis: 600, hoechstpreis: 660, medianPrice: 650, isNewAllTimeLow: false, dataPointCount: 10, time: Date.now() }));
        window.ToppreiseSuite?.memoryCache?.set('106', JSON.parse(JSON.stringify({ tiefstpreis: 600, hoechstpreis: 660, medianPrice: 650, isNewAllTimeLow: false, dataPointCount: 10, time: Date.now() }))); // ~4%
    ...
    assert page.evaluate(
        "() => document.getElementById('product-row-2').style.display === 'none'"
    )
    assert page.evaluate(
        "() => document.getElementById('product-row-3').style.display === 'none'"
    )
    ...
    # Row 1 restored
    row1_cards = page.evaluate(
        "() => Array.from(document.querySelectorAll('#product-row-1 .Plugin_Product')).map(c => c.id)"
    )
    assert row1_cards == ["multi-card-1", "multi-card-2"]

    # Row 2 restored
    row2_cards = page.evaluate(
        "() => Array.from(document.querySelectorAll('#product-row-2 .Plugin_Product')).map(c => c.id)"
    )
    assert row2_cards == ["multi-card-3", "multi-card-4"]

    # Row 3 restored
    row3_cards = page.evaluate(
        "() => Array.from(document.querySelectorAll('#product-row-3 .Plugin_Product')).map(c => c.id)"
    )
...
def test_bestpreise_mode_all_cards_remain_visible_when_uncached(page: Page):
    ...
    for cv in card_visibilities:
        assert cv["hasOffsetParent"] is True, (
            f"Card {cv['id']} has null offsetParent (invisible)"
        )
        assert cv["computedDisplay"] != "none", f"Card {cv['id']} has display: none"
        assert cv["hasBestpreiseHiddenClass"] is False, (
            f"Card {cv['id']} has tp-bestpreise-hidden"
        )
        assert cv["hiddenAncestor"] is None, (
            f"Card {cv['id']} has hidden ancestor: {cv['hiddenAncestor']}"
        )
...
def test_bestpreise_mode_progressive_reveal(page: Page):
    ...
    visible_count_1 = page.evaluate(
        "() => Array.from(document.querySelectorAll('.Plugin_Product')).filter(c => c.offsetParent !== null).length"
    )
    ...
    first_card_id = page.evaluate(
        "() => Array.from(document.querySelectorAll('.Plugin_Product')).filter(c => c.offsetParent !== null)[0].id"
    )
    assert first_card_id == "card-cheapest"

    # All 5 cards still visible (1 deal + 4 unscanned)
    visible_count_2 = page.evaluate(
        "() => Array.from(document.querySelectorAll('.Plugin_Product')).filter(c => c.offsetParent !== null).length"
    )
    ...
    visible_cards = page.evaluate(
        "() => Array.from(document.querySelectorAll('.Plugin_Product')).filter(c => c.offsetParent !== null).map(c => c.id)"
    )
    assert visible_cards == [
        "card-cheapest",
        "card-competing-reference",
        "card-negative",
        "card-cat-excluded",
        "card-low-offers",
    ]
    ...
    visible_cards_after = page.evaluate(
        "() => Array.from(document.querySelectorAll('.Plugin_Product')).filter(c => c.offsetParent !== null).map(c => c.id)"
    )
    assert visible_cards_after == [
        "card-cheapest",
        "card-competing-reference",
        "card-cat-excluded",
        "card-low-offers",
...
def test_column_wrapper_layout_fidelity_and_hiding(page: Page):
    ...
    page.evaluate("""() => {
        ...
        window.ToppreiseSuite?.memoryCache?.set('88802', JSON.parse(JSON.stringify({
            tiefstpreis: 100,
            hoechstpreis: 250,
            medianPrice: 150,
            isNewAllTimeLow: false,
    ...
    assert page.evaluate(
        "() => window.getComputedStyle(document.getElementById('wrapper-col-2')).display === 'none'"
    )
...
def test_deal_score_weight_slider_zero_persistence(page: Page):
    ...
    stored_weight = page.evaluate(
        "() => window.ToppreiseSuite.CONFIG.BESTPREISE_WEIGHT_RECORD"
    )
...
def test_deal_score_weight_preset_dropdown_in_filter_bar(page: Page):
    ...
    page.evaluate(
        "document.querySelector('#tp-weight-popover button[data-weight=\"0.00\"]').click()"
    )
    assert page.evaluate(
        "() => window.ToppreiseSuite.CONFIG.BESTPREISE_WEIGHT_RECORD === 0.0"
    )
